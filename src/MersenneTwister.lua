--- 
-- @author Spencer Tupis, KillaVanilla
-- @copyright 2014-2015 Spencer Tupis, KillaVanilla
-- @release Module$Core$Module$MersenneTwister$ModuleScript
-- @class module
...
local MTRNG = {};
local bit=require("bit"); -- Lua BitOp;
-- Helper functions:
...
function MTRNG:initalize_mt_generator(seed)
  index = 0
  MT[0] = seed
  for i=1, 623 do
...
function MTRNG:generate_mt() -- Restock the MT with new random numbers.
  for i=0, 623 do
...
function MTRNG:extract_mt(min, max) -- Get one number from the Mersenne Twister.
  if index == 0 then
    generate_mt()
  end
  local y = MT[index]
...
function MTRNG:seed_from_mt(seed) -- seed ISAAC with numbers from the MT:
  if seed then
    mtSeeded = false
    mtSeed = seed
  end
  if not mtSeeded or (math.random(1, 100) == 50) then -- Always seed the first time around. Otherwise, seed approximately once per 100 times.
    MTRNG.initalize_mt_generator(mtSeed)
    mtSeeded = true
    mtSeed = MTRNG.extract_mt()
  end
  for i=1, 256 do
    randrsl[i] = MTRNG.extract_mt()
  end
end
...
function MTRNG:generate_isaac(entropy)
  aa = 0
  bb = 0
  cc = 0
...
local function getRandom()
  if #mm > 0 then
    ...
    MTRNG.generate_isaac()
    return table.remove(mm, 1)
  end
end

function MTRNG:random(min, max)
  if not max then
    max = 2^32-1
  end
  if not min then
