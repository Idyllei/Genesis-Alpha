...
do
  ...
  class SignalMaid
      ...
      if not sEvtName-- Make sure the LuaSignal was given a name.
      	error "Attempt to create unnamed LuaSignal in SignalMaid", 2
      	return
      if @@__SIGNALS[sEvtName] -- Make sure we aren't going to overwrite an already created LuaSignal
      	print "[WARN] [SignalMaid] Attempt to create a LuaSignal with a name already used as index. Skipping."
        return
